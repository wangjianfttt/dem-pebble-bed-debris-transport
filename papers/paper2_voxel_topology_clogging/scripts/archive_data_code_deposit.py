#!/usr/bin/env python3
"""Create a compressed archive for the staged Paper 2 lightweight deposit."""
...
DEFAULT_STAGING_DIR = PAPER_DIR / "deposit" / "paper2_lightweight_deposit"
DEFAULT_ARCHIVE_DIR = PAPER_DIR / "deposit" / "archives"
ARCHIVE_STEM = "paper2_lightweight_deposit"


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--staging-dir", type=Path, default=DEFAULT_STAGING_DIR, help="Directory produced by stage_data_code_deposit.py.")
...
def iter_staged_files(staging_dir: Path) -> list[Path]:
    """Return all files in the staging directory in stable archive order."""
...
def create_zip_archive(staging_dir: Path, archive_path: Path, force: bool = False) -> dict[str, object]:
    """Create a zip archive from a staged deposit directory."""
